"""
02_advanced_cleaning_and_feature_engineering.py (OPTIMIZED VERSION)

PRODUCTION PIPELINE: Performs advanced cleaning, imputation, validation, and feature engineering.
OPTIMIZED FOR SPEED: Vectorized operations, optional spaCy, parallel processing, progress tracking.

Expected runtime: 2-5 minutes (down from 8-15 minutes)
...
USE_PARALLEL = True  # Set to True to use multiprocessing (faster)
...
def remove_trailing_code_in_parens(name_s: pd.Series, code_s: pd.Series) -> pd.Series:
    """Vectorized removal of trailing codes in parentheses."""
...
def build_operation_legend(df, code_col="SURGICAL_CODE", nature_col="NATURE", 
    ...
    """Build operation legend (vectorized)."""
    work = pd.DataFrame({
...
# ============================================================================
# HELPER FUNCTIONS: DATE/TIME IMPUTATION (OPTIMIZED)
# ============================================================================

def enforce_ordering_vectorized(data, col_pairs):
    """Vectorized enforcement of time ordering."""
...
def impute_planned_from_knife_vectorized(data):
    """Vectorized imputation of planned times from knife-to-skin."""
...
def impute_patient_fetch_time_vectorized(data):
    """Vectorized imputation of patient fetch time as midpoint."""
...
def compute_statistical_marks(data):
    """Compute average 'marks' for imputing missing times (optimized)."""
    marks = {}
...
def impute_induction_prep_reversal_cleanup_vectorized(data, marks):
    """Vectorized imputation using statistical marks."""
...
def validate_datetime_order_vectorized(data, cols):
    """
    Vectorized validation of datetime ordering.
...
def validate_duration_vectorized(data, cols, max_hours=72):
    """
    Vectorized validation of duration limits.
    Returns boolean mask of valid rows.
    """
    times = data[cols]
    valid = pd.Series(True, index=data.index)
...
# ============================================================================
# HELPER FUNCTIONS: DELAY REASON TAXONOMY (OPTIMIZED)
# ============================================================================
...
# Simplified taxonomy (top categories only for speed)
TAXONOMY_DATA_FAST = [
...
def process_delay_reasons_vectorized(data):
    ...
    # Flag Reason_Is_Late (vectorized)
...
def convert_to_correct_dtypes(data):
    """
    Convert all columns to their correct data types for analysis.
    Returns the dataframe with corrected dtypes.
    """
    print("Converting data types...")
    
    # 1. DATETIME COLUMNS (already handled, but ensure consistency)
    datetime_cols = [
        "PLANNED_PATIENT_CALL_TIME", "PLANNED_PATIENT_FETCH_TIME",
        "PLANNED_RECEPTION_IN_TIME", "PLANNED_ENTER_OR_TIME",
        "PLANNED_ANAESTHESIA_INDUCTION", "PLANNED_SURGERY_PREP_TIME",
    ...
    # 2. NUMERIC COLUMNS (durations and delays in minutes)
    numeric_cols = [
...
def main():
    ...
    if remaining_missing.sum() > 0:
        print(f"\n  ℹ️  Remaining missing values in non-critical columns:")
