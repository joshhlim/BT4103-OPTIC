"""
01_standardize_raw_data.py (OPTIMIZED VERSION)
...
Output: Standardized_Raw_Dataset.csv (Simulation of a raw dataset with proper data entr protocols)
"""
import pandas as pd
import numpy as np
import re
...
# Valid date range for this historical dataset
MIN_DATE = pd.Timestamp("2016-12-31")
MAX_DATE = pd.Timestamp("2022-02-25")

# Planned datetime columns
PLANNED_COLS = [
...
def is_time_only_string(s: str) -> bool:
    """Check if string is time-only (e.g., '08:15' or '8:15:00 AM')."""
    if not isinstance(s, str):
        return False
...
def build_validation_lookup_dict(validation_df, cols):
    ...
    This avoids repeated dataframe filtering in the main loop.
    """
    lookup = {}
...
def attach_dates_to_time_only_strings(row, planned_cols, actual_cols, planned_lookup, actual_lookup):
    """
    OPTIMIZED: Uses pre-built lookup dictionaries instead of repeated dataframe filtering.
    ...
    # Fix planned_start if out of range (using lookup dict)
    ...
    # Fix actual_start if out of range (using lookup dict)
    ...
    # Fallback: if actual_start missing, use planned_start
    ...
    # Process actual columns (same logic)
...
def sync_columns_vectorized(data, col_a, col_b):
    """Vectorized column syncing (much faster than apply)."""
    a = data[col_a]
    b = data[col_b]
...
def main():
    ...
    print("=" * 80)
    print("OPTIMIZED STANDARDIZATION PIPELINE - File 1".center(80))
    print("=" * 80)
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("STEP 2: STANDARDIZE TEXT FIELDS (VECTORIZED)")
    print("=" * 80)
    step_start = time.time()
    
    # Emergency Priority standardization (vectorized)
    ...
    # AOH standardization (vectorized)
    ...
    # ADMISSION_STATUS: Remove numeric entries (vectorized)
    ...
    # CANCER_INDICATOR standardization (vectorized)
    ...
    # TRAUMA_INDICATOR standardization (vectorized)
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("STEP 3: HANDLE OPTIONAL COLUMNS (VECTORIZED)")
    print("=" * 80)
    step_start = time.time()
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("STEP 4: FIX DATE/TIME COLUMNS (OPTIMIZED)")
    print("=" * 80)
    step_start = time.time()
    
    print("Pre-building validation lookup dictionaries...")
    # Build lookup dictionaries once (much faster than repeated filtering)
    ...
    # Convert all datetime columns to proper datetime dtype (vectorized)
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("STEP 5: SIMPLE COLUMN SYNCING (VECTORIZED)")
    print("=" * 80)
    step_start = time.time()
    
    # Sync patient call/fetch times (vectorized)
    ...
    # Sync cleanup/exit times (vectorized)
    ...
    # Copy ACTUAL_RECEPTION_IN_TIME to PATIENT_CALL_TIME if missing (vectorized)
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("STEP 6: HANDLE ADMISSION-RELATED COLUMNS (VECTORIZED)")
    print("=" * 80)
    step_start = time.time()
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("STEP 7: HANDLE CLINICAL STAFF COLUMNS (VECTORIZED)")
    print("=" * 80)
    step_start = time.time()
